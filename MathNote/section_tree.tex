...
\begin{lemma}
假设定义在非负整数上的函数$f(n)$满足$nf(n) = (n+1)f(n-1) + cn$, 那么$f(n) = \frac{(n+1)}{2} f(1) + (n+1)\prt{\log_e(n+1) + \g - \frac{3}{2}}$
\end{lemma}
...
\begin{eqnarray*}
f(n) = \frac{(n+1)}{2} f(1) + (n+1)\prt{\log_e(n+1) + \g - \frac{3}{2}}
\end{eqnarra*}
